...
// Right tape — paths mirrored within the SVG coordinate space so that
// CSS positioning can use the same simple scale(0.75) trick as the left tape.
// translate(95,0) scale(-1,1) reflects around x=0 then shifts back into view.
const tapeRight = (
  <svg xmlns="http://www.w3.org/2000/svg" width="95" height="80" viewBox="0 0 95 80" fill="none">
    <g transform="translate(95,0) scale(-1,1)">
      {tapePaths}
    </g>
  </svg>
);
...
export function TapedFooter({ className }: { className?: string }) {
  ...
  return (
    <footer
      id="about"
      className={cn('my-8 px-4 max-w-7xl mx-auto', className)}
      style={{ fontFamily: "'DM Sans', sans-serif" }}
    >
      {/* ── Card with tape corners ── */}
      {/* overflow-visible so the tape pieces can bleed outside the card edges */}
      <div className="relative bg-white rounded-3xl max-w-7xl mx-auto px-4 py-10 flex flex-col md:flex-row justify-between items-start gap-6 shadow-sm border border-[#0a1f3d]/8 overflow-visible">

        {/* Tape top-left — scale 75% from the top-left corner */}
        <div
          className="hidden md:block absolute -top-5 -left-7 opacity-90 pointer-events-none"
          style={{ transform: 'scale(0.75)', transformOrigin: 'top left' }}
        >
          {tape}
        </div>

        {/* Tape top-right — SVG paths are pre-mirrored inside the SVG viewBox,
            so we can use the same simple scale(0.75) from top-right as the left
            tape uses from top-left. No CSS flip = no off-screen displacement bug. */}
        <div
          className="hidden md:block absolute -top-5 -right-7 opacity-90 pointer-events-none"
          style={{ transform: 'scale(0.75)', transformOrigin: 'top right' }}
        >
          {tapeRight}
        </div>

        <div className="flex flex-col md:flex-row items-start justify-between gap-8 md:gap-10 px-2 md:px-8 flex-1">
          ...
          <div className="flex flex-col items-start gap-3 w-full md:max-w-xs">
            ...
            <div className="flex items-center gap-2">
              ...
              <span
                ...
                style={{ color: TEAL }}
              >
                All codes verified · {currentMonth} {currentYear}
              </span>
            </div>
          ...
          <div className="grid grid-cols-2 sm:flex sm:flex-row gap-8 md:gap-16 items-start w-full sm:w-auto">
      ...
      {/* ── Bottom bar ── */}
      <div
        className="mt-3 px-4 md:px-8 flex flex-col md:flex-row md:justify-between md:items-center gap-1.5 md:gap-2"
        style={{ color: MUTED }}
      >
        {/* Copyright */}
        <p className="text-[11px] md:text-sm font-light">
          ©{currentYear} Anphonic Shop · Vol. 01
        </p>
